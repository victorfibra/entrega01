...
@Service("pfService") // Isso avisa ao Spring que esta classe é um componente de serviço [cite: 395]
public class TrafegoPFServiceImpl implements TrafegoService {
    ...
    public ContratoResponseDTO analisarContrato(Contrato contrato) {
        ...
        if (contrato.getTotalTrafegoGB() < 10.0) {
            status = StatusConsumo.BAIXO_CONSUMO; // Possível ONU/Roteador travado
        } else if (contrato.getTotalTrafegoGB() > 500.0) {
